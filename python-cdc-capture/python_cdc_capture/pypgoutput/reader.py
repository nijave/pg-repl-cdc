...
from .utils import SourceDBHandler
...
def map_tuple_to_dict(
    tuple_data: decoders.TupleData, relation: TableSchema
...
def convert_pg_type_to_py_type(pg_type_name: str) -> type:
    if (
        pg_type_name == "bigint"
        or pg_type_name == "integer"
        or pg_type_name == "smallint"
    ):
        return int
    elif (
        pg_type_name == "timestamp with time zone"
        or pg_type_name == "timestamp without time zone"
    ):
        return datetime
        # json not tested yet
...
class LogicalReplicationReader:
    ...
    def __init__(
        ...
        **kwargs: typing.Optional[str],
        ...
        self.table_schemas: typing.Dict[
            int, TableSchema
        ] = dict()  # map relid to table schema
    ...
    def transform_raw(
        ...
        for msg in message_stream:
            ...
            # message processors below will throw an error if transaction_metadata doesn't exist
                ...
                yield from self.process_truncate(
                    message=msg, transaction=transaction_metadata
                )
            elif message_type == "C":
    ...
    def process_insert(
        ...
        after = map_tuple_to_dict(
            tuple_data=decoded_msg.new_tuple, relation=self.table_schemas[relation_id]
        )
        return ChangeEvent(
    ...
    def process_update(
        ...
        if decoded_msg.old_tuple:
            before_raw = map_tuple_to_dict(
                tuple_data=decoded_msg.old_tuple,
                relation=self.table_schemas[relation_id],
            )
    ...
    def process_delete(
        ...
        relation_id: int = decoded_msg.relation_id
        before_raw = map_tuple_to_dict(
            tuple_data=decoded_msg.old_tuple, relation=self.table_schemas[relation_id]
        )
